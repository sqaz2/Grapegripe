# Historical campaign design reference

Archived on 9 September 2026. This preserves the original brief; its build order and prompt are not current instructions. See SOL-HANDOFF.md and SOL-NEXT.md for active work.

# Grape Gripe — Sol build handoff

**Current direction (2026-09-05):** Read [UNIVERSE-SCOPE.md](UNIVERSE-SCOPE.md) and [SOL-NEXT.md](SOL-NEXT.md) first. The user now wants one persistent character across StarMuff's games and endings built incrementally. Schema-2 character/memory/adventure separation is implemented in this checkout. The document below is historical campaign context; its old build order and copy/paste prompt are superseded.
